...
{
	return (uint32_t)DMA_GetFlagStatus(DMA2_FLAG_TC4);	//Channel4 Tranfer Complete Flag
}
...
void SD_DMA_RxConfig(uint32_t *BufferDST, uint32_t BufferSize)
{
	...
	//clear the DMA2_Channel4 Flag
	DMA_ClearFlag(DMA2_FLAG_TE4 | DMA2_FLAG_TC4 | DMA2_FLAG_HT4 | DMA2_FLAG_GL4);

	//Disable the DMA2_Channel4
	DMA_Cmd(DMA2_Channel4, DISABLE);

	//DMA2_Channel Config
	DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)SDIO_FIFO_ADDRESS;		//外设地址
	...
	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;						//存储目的地址自增
	DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Word;		//设置数据大小为字
	DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Word;			
	DMA_InitStructure.DMA_Mode = DMA_Mode_Normal;					//不循环模式
	DMA_InitStructure.DMA_Priority = DMA_Priority_High;				//优先级为高
	DMA_InitStructure.DMA_M2M = DMA_M2M_Disable;					//非存储器转存储器
	DMA_Init(DMA2_Channel4, &DMA_InitStructure);
	
	//Enable the DMA2_Channel4
...
void SD_DMA_TxConfig(uint32_t *BufferSRC, uint32_t BufferSize)
{
	...
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralDST;							//外设为源地址
	DMA_InitStructure.DMA_BufferSize = BufferSize / 4;							//1/4缓存大小
	DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;			//外设地址不自增
	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;						//存储目的地址自增
	DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Word;		//设置数据大小为字
...
{ 
	...
	//Disable the RCC_AHBPeriph_SDIO 
	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_SDIO, DISABLE);
	//Set the PC8,9,10,11,12 GPIO_Mode_IN_FLOATING
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8 | GPIO_Pin_9 | GPIO_Pin_10| GPIO_Pin_11| GPIO_Pin_12;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
	GPIO_Init(GPIOC, &GPIO_InitStructure);
	//Set the PD2 to GPIO_Mode_IN_FLOATING
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;
	GPIO_Init(GPIOD, &GPIO_InitStructure);
...
SD_Error SD_ReadBlock(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize)
{
  ...
  //Add flowing programming to resove DMA interrupt
...
SD_Error SD_WriteMultiBlocks(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  //Add flowing programming to while DMA check interrupt
  SDIO_CmdInitStructure.SDIO_Argument = (uint32_t)BlockSize;
  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SET_BLOCKLEN;
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
